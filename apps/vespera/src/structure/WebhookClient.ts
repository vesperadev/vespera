...
import { WebhooksAPI } from '../core';
import type { RawFile } from '../rest';
import { REST } from '../rest';

import { VesperaError } from './Error';

export type WebhookClientOptions<T extends string | { id: string; token: string }> = T;
...
export class WebhookClient<T extends string | { id: string; token: string }> {
   ...
   * @param {REST} [rest] - The REST instance to use.
   */
  public constructor(options: WebhookClientOptions<T>, rest?: REST) {
    this.API = new WebhooksAPI(rest ?? new REST({ version: '10' }));

    const id = typeof options === 'string' ? options.split('/')[options.split('/').length - 2] : options.id;

    if (!id) {
      throw new VesperaError('Webhook ID is required');
    }

    const token = typeof options === 'string' ? options.split('/').pop() : options.token;

    if (!token) {
      throw new VesperaError('Webhook Token is required');
    }
   ...
   */
  public async edit({ reason, ...data }: RESTPatchAPIWebhookJSONBody & { reason: string }): Promise<APIWebhook> {
   ...
   */
  public async deleteMessage(messageId: string, options?: { thread_id: string }): Promise<void> {
   ...
   */
  public async fetchMessage(messageId: string, query?: RESTGetAPIWebhookWithTokenMessageQuery): Promise<APIMessage> {
